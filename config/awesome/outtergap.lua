...
local function tile_group(cls, wa, orientation, fact, group)
    local border_gap_width = 200
    local border_gap_height = 400
    ...
    local available = wa[width] - (group.coord - wa[x]) - border_gap_width*2
    ...
    local coord = wa[y] + border_gap_height
    local geom = {}
    local used_size = 0
    local unused = wa[height] - border_gap_height*2
    for c = group.first,group.last do
        local i = c - group.first +1
        geom[width] = size - cls[c].border_width * 2
        geom[height] = math.floor(unused * fact[i] / total_fact) - cls[c].border_width * 2
        geom[x] = group.coord + border_gap_width
        geom[y] = coord
